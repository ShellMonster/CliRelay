...
const normalizeOpenAIForm = (form?: Partial<OpenAIFormState> | null): OpenAIFormState => {
  const apiKeyEntries = Array.isArray(form?.apiKeyEntries)
    ? form.apiKeyEntries
        .map((entry) => ({
        ...
        .filter((entry) => entry.apiKey !== "" || entry.proxyUrl !== "")
    : [];

  const modelEntries = Array.isArray(form?.modelEntries)
    ? form.modelEntries
        .map((entry) => ({
  ...
  return {
    ...
    headers: Array.isArray(form?.headers)
      ...
      : [],
    apiKeyEntries: apiKeyEntries.length ? apiKeyEntries : [buildApiKeyEntry()],
    modelEntries: modelEntries.length ? modelEntries : [{ name: "", alias: "" }],
    testModel: typeof form?.testModel === "string" ? form.testModel : undefined,
  };
};
...
export function AiProvidersOpenAIEditLayout() {
  ...
  const form = useMemo(() => normalizeOpenAIForm(draft?.form ?? buildEmptyForm()), [draft?.form]);
    ...
    if (initialData) {
      ...
      const initialTestModel =
        initialData.testModel && available.includes(initialData.testModel)
          ...
          : available[0] || "";
      initDraft(draftKey, {
        form: normalizeOpenAIForm(seededForm),
      ...
      initDraft(draftKey, {
        form: normalizeOpenAIForm(buildEmptyForm()),
        testModel: "",
        testStatus: "idle",
        testMessage: "",
        keyTestStatuses: [],
