...
import { RefreshCw, Cpu, DollarSign, Activity } from "lucide-react";
            ...
            <table className="w-full min-w-[1000px] table-fixed border-separate border-spacing-0 text-sm">
              <thead className="bg-white/95 backdrop-blur dark:bg-neutral-950/75">
                <tr className="h-11 text-left text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-white/55">
                  <th className="w-64 border-b border-slate-200 px-4 dark:border-neutral-800">
                    模型
                  </th>
                  <th className="w-20 border-b border-slate-200 px-4 text-right dark:border-neutral-800">
                    请求数
                  </th>
                  <th className="w-20 border-b border-slate-200 px-4 text-right dark:border-neutral-800">
                    成功
                  ...
                  <th className="w-32 border-b border-slate-200 px-4 dark:border-neutral-800">
                    最后使用
                  </th>
                  <th className="w-20 border-b border-slate-200 px-4 dark:border-neutral-800">
                    定价
                  </th>
                </tr>
              </thead>
            ...
            <div className="rounded-xl border border-indigo-200 bg-indigo-50/50 px-4 py-3 dark:border-indigo-800 dark:bg-indigo-950/30">
              ...
              {pricingModelStats && (
                <div className="mt-1 text-xs text-indigo-700/70 dark:text-indigo-400/60">
                  输入 {formatNumber(pricingModelStats.inputTokens)} · 输出{" "}
                  {formatNumber(pricingModelStats.outputTokens)} · 缓存{" "}
                  {formatNumber(pricingModelStats.cachedTokens)} Token
                </div>
              )}
            </div>
          )}
              ...
              <div className="mt-1 text-lg font-bold tabular-nums text-emerald-700 dark:text-emerald-400">
                {formatCurrency(
                  (pricingModelStats.inputTokens / 1_000_000) * (parseFloat(editInputPrice) || 0) +
                    (pricingModelStats.outputTokens / 1_000_000) *
                      (parseFloat(editOutputPrice) || 0) +
                    (pricingModelStats.cachedTokens / 1_000_000) *
                      (parseFloat(editCachedPrice) || 0),
                )}
              </div>
            </div>
          )}
