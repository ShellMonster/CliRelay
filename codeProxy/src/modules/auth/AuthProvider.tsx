...
// 简单的加密工具：使用 btoa/atob 进行基础编码，增加前缀避免明文存储
const STORAGE_PREFIX = "cpa:";

const encodeKey = (key: string): string => {
  try {
    return STORAGE_PREFIX + btoa(key);
  } catch {
    return key;
  }
};
...
const decodeKey = (encoded: string): string => {
  if (!encoded.startsWith(STORAGE_PREFIX)) {
    return encoded; // 兼容旧数据
  }
  try {
    return atob(encoded.slice(STORAGE_PREFIX.length));
  } catch {
    return encoded;
  }
};
...
const readAuthSnapshot = (): AuthSnapshot | null => {
  try {
    ...
    return {
      ...
      managementKey: decodeKey(parsed.managementKey),
...
const writeAuthSnapshot = (snapshot: AuthSnapshot): void => {
  const encryptedSnapshot = {
    ...snapshot,
    managementKey: encodeKey(snapshot.managementKey),
  };
  localStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify(encryptedSnapshot));
};
