...
export function ApiKeysPage() {
    ...
    const loadModels = useCallback(async (apiKeyEntries: ApiKeyEntry[]) => {
        try {
            ...
            try {
                const list = await fetchModelsFromStore(serverBase, apiKey, true);
                if (Array.isArray(list) && list.length > 0) {
                    const opts: MultiSelectOption[] = list
    ...
    useEffect(() => {
        if (!storeModels.length) return;
        const opts: MultiSelectOption[] = storeModels
            .map((model) => ({ value: model.name, label: model.name }))
            .filter((item) => item.value)
            .sort((a, b) => a.label.localeCompare(b.label));
        setAvailableModels((prev) => (prev.length >= opts.length ? prev : opts));
    }, [storeModels]);
    ...
    }, [availableModels, form.allowedModels]);
