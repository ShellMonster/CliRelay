...
export function FailureAnalysis({
  ...
  const failureStats = useMemo(() => {
    const monitorStats = data?.monitor?.failureStats;
    ...
    monitorStats.channels.forEach((channel) => {
      ...
      stats[displayName] = {
        ...
        maskedKey: masked,
        failedCount: channel.failed_requests,
        lastFailTime: channel.last_failed_at ? new Date(channel.last_failed_at).getTime() : 0,
        models: {},
      };
    });
    ...
    monitorStats.models.forEach((modelPoint) => {
      ...
      channel.models[modelPoint.model] = {
        ...
        failure: modelPoint.failed_requests,
    ...
    return Object.values(stats)
      ...
      .slice(0, 10);
  }, [data?.monitor?.failureStats, providerMap]);
