...
import { usageApi, authFilesApi } from "@/lib/http/apis";
...
export function RequestLogs({
  ...
  // 日志独立数据状态
  const [logItems, setLogItems] = useState<UsageLogItem[]>([]);
  ...
  const fetchLogData = useCallback(async () => {
    ...
    try {
      const response = await usageApi.getUsageLogs({
        page: 1,
        size: 200,
        days: typeof timeRange === "number" ? timeRange : 7,
        api_key: apiFilter || undefined,
      });
      setLogItems(
        (response.items as UsageLogItem[]).filter((item) => {
          return !apiFilter || item.api_key.toLowerCase().includes(apiFilter.toLowerCase());
        }),
      );
    } catch (err) {
  ...
  }, [logItems, providerMap, providerTypeMap]);
