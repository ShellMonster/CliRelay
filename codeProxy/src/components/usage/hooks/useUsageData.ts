...
import { usageApi, type UsageOverviewResponse } from "@/lib/http/apis/usage";
...
export function useUsageData(): UseUsageDataReturn {
  ...
  const loadUsage = useCallback(async () => {
    ...
    setError("");
    try {
      const overview = await usageApi.getUsageOverview(30);

      setUsage({
        total_requests: overview.summary.TotalRequests,
        success_count: overview.summary.SuccessRequests,
        failure_count: overview.summary.FailedRequests,
        total_tokens: overview.summary.TotalTokens,
        overview,
      });
