...
const normalizeModelAliases = (models: unknown): ModelAlias[] => {
  ...
  return models
    .map((item) => {
      ...
      if (alias && alias !== name) entry.alias = String(alias);
      if (priority !== undefined) {
        const parsed = Number(priority);
        if (Number.isFinite(parsed)) entry.priority = parsed;
      }
...
const normalizeUserAgentRoutingRules = (input: unknown): UserAgentRoutingRuleConfig[] => {
  ...
  input.forEach((item) => {
    ...
    const matchMode =
      item["match-mode"] === "regex" || item.matchMode === "regex" ? "regex" : "contains";
    normalized.push({
...
const normalizeGeminiKeyConfig = (item: unknown): GeminiKeyConfig | null => {
  ...
  if (!apiKey && typeof item === "string") apiKey = item;
  const trimmed = String(apiKey || "").trim();
  if (!trimmed) return null;
...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  if (typeof requestRetry === "number" && Number.isFinite(requestRetry)) {
    ...
    if (Number.isFinite(parsed)) config.requestRetry = parsed;
  }
  ...
  const copilotCompatList =
    raw["copilot-compat-api-key"] ?? raw.copilotCompatApiKey ?? raw.copilotCompatApiKeys;
