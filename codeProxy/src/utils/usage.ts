...
const getEntries = (usageData: unknown): FlatUsageEntry[] | null => {
  const usageRecord = isRecord(usageData) ? usageData : null;
  const entriesRaw = usageRecord?.entries;
  if (!Array.isArray(entriesRaw)) return null;
  return entriesRaw as FlatUsageEntry[];
};

const getOverview = (usageData: unknown): UsageOverviewResponse | null => {
  const usageRecord = isRecord(usageData) ? usageData : null;
  const overview = usageRecord?.overview;
  return overview && isRecord(overview) ? (overview as unknown as UsageOverviewResponse) : null;
};

// Legacy support for imported usage snapshots that still persist the nested apis/models/details shape.
...
export function filterUsageByTimeRange<T>(
  ...
  if (entries) {
    ...
    const failureCount = filteredEntries.reduce((sum, entry) => sum + (entry.failed ? 1 : 0), 0);
    const successCount = totalRequests - failureCount;
    const totalTokens = filteredEntries.reduce(
      (sum, entry) => sum + (Number(entry.totalTokens) || 0),
      0,
    );

    return {
      ...(isRecord(usageData) ? usageData : {}),
  ...
  if (entries) {
    return entries
      .map((entry) => ({
        ...
        source: normalizeUsageSourceId(entry.source),
        auth_index: Number(entry.authIndex) || 0,
        failed: Boolean(entry.failed),
        __modelName: entry.model,
        tokens: {
      ...
      .filter((detail) => typeof detail.timestamp === "string");
  }
...
export function getModelNamesFromUsage(usageData: unknown): string[] {
  const overview = getOverview(usageData);
  if (overview) {
...
export function getApiStats(
  ...
  if (entries) {
    ...
    entries.forEach((entry) => {
      ...
      if (entry.failed) apiBucket.failureCount += 1;
      else apiBucket.successCount += 1;
      ...
      apiBucket.models[modelName].requests += 1;
      apiBucket.models[modelName].tokens += Number(entry.totalTokens) || 0;
      if (entry.failed) apiBucket.models[modelName].failureCount += 1;
      else apiBucket.models[modelName].successCount += 1;

      apiBucket.totalCost += calculateCost(
        {
          timestamp: entry.timestamp,
          source: entry.source,
          auth_index: Number(entry.authIndex) || 0,
      ...
      apiMap.set(endpoint, apiBucket);
    });

    return Array.from(apiMap.entries()).map(([endpoint, stats]) => ({
      endpoint,
...
export function getModelStats(
  ...
  const overview = getOverview(usageData);
  if (overview) {
    return overview.models.map((item) => ({
      model: item.model,
  ...
  if (entries) {
    ...
    entries.forEach((entry) => {
      ...
      if (entry.failed) bucket.failureCount += 1;
...
export function buildHourlySeriesByModel(
  ...
  const overview = getOverview(usageData);
  if (overview) {
    const points = overview.request_trend.slice(-hourWindow);
    const labels = points.map((point) => point.bucket);
    const allSeries = points.map((point) =>
      metric === "tokens" ? point.total_tokens : point.requests,
    );
    const dataByModel = new Map<string, number[]>();
    dataByModel.set("All Models", allSeries);
    return { labels, dataByModel, hasData: allSeries.some((value) => value > 0) };
  }
...
export function buildDailySeriesByModel(
  ...
  const overview = getOverview(usageData);
  if (overview) {
    const labels = overview.request_trend.map((point) => point.bucket);
    const allSeries = overview.request_trend.map((point) =>
      metric === "tokens" ? point.total_tokens : point.requests,
    );
    const dataByModel = new Map<string, number[]>();
    dataByModel.set("All Models", allSeries);
...
export function calculateServiceHealthDataFromOverview(
  ...
  overview.service_health.forEach((item) => {
    ...
    else if (failure === 0) blocks.push("success");
    else if (success === 0) blocks.push("failure");
    else blocks.push("mixed");

    const start = Date.parse(item.bucket);
    const startTime = Number.isNaN(start) ? 0 : start;
    blockDetails.push({
      success,
      failure,
...
export function computeKeyStats(
  ...
  if (entries) {
    ...
    entries.forEach((entry) => {
      const source = normalizeUsageSourceId(entry.source, masker);
      const authIndex = normalizeAuthIndex(entry.authIndex);
      if (source) {
        const bucket = ensureBucket(bySource, source);
        if (entry.failed) bucket.failure += 1;
        else bucket.success += 1;
      }
      if (authIndex) {
        const bucket = ensureBucket(byAuthIndex, authIndex);
        if (entry.failed) bucket.failure += 1;
        else bucket.success += 1;
      }
    });
...
export function buildHourlyTokenBreakdown(
  ...
  const overview = getOverview(usageData);
  if (overview) {
    const points = overview.token_breakdown.slice(-hourWindow);
    return {
      labels: points.map((point) => point.bucket),
      dataByCategory: {
        input: points.map((point) => point.input_tokens),
        output: points.map((point) => point.output_tokens),
        cached: points.map((point) => point.cached_tokens),
        reasoning: points.map((point) => point.reasoning_tokens),
      },
      hasData: points.some((point) => point.total_tokens > 0),
    };
  }
...
export function buildDailyTokenBreakdown(usageData: unknown): TokenBreakdownSeries {
  const overview = getOverview(usageData);
  if (overview) {
    const points = overview.token_breakdown;
    return {
      labels: points.map((point) => point.bucket),
      dataByCategory: {
        input: points.map((point) => point.input_tokens),
