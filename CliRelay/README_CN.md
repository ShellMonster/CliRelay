...
如果你的网络访问 Docker Hub 较慢，可以使用预配置的中国镜像源：
...
| 配置项 | 默认值 | 说明 |
|:-------|:-------|:-----|
| `DOCKER_REGISTRY` | `docker.1ms.run/` | Docker Hub 镜像源 |
| `APK_MIRROR` | `mirrors.aliyun.com` | Alpine apk 镜像源 |
...
**注意事项：**
- 请确保使用 `docker compose build` 构建，以正确读取 `.env` 文件中的镜像源配置
- 如需禁用镜像源，将对应配置项留空即可
