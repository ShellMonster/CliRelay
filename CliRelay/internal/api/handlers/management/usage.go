...
func (h *Handler) GetUsageStatistics(c *gin.Context) {
	...
	summary, err := usage.QueryUsageSummary()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	snapshot.TotalRequests = summary.TotalRequests
	snapshot.SuccessCount = summary.SuccessCount
	snapshot.FailureCount = summary.FailureCount
	snapshot.TotalTokens = summary.TotalTokens
